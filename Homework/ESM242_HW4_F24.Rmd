...
<span style="color: blue;">**The final values of  p_T and R_T should be 40 and 0 respectively. To maximize the present discounted profits from resource extraction**</span>

```{r}
normal_solution <- function(guess, a, b, c, discount, Ro) {
  L=guess
  t=seq(from=0,to=L)
  ...
  return(data.frame("Time" = t, "Price"=P, "Quantity"=choice, "Reserves" = round(R), "Profits" = benefits, "Lambda"=lambda, "PV"=pv)
    )
}

options(scipen=999)
df <- normal_solution(36, 40, 1, 0, 0.05, 778.1259)

knitr::kable(df, align="c")
```


# Looking at Results from Table output 
...
opt_fct <- function(choice, int_state, a, b, c, discount) {
  ...
  state=vector(mode="numeric", length=0) # State is reserves
  benefits=vector(mode="numeric", length=0)
  # lambda=vector(mode="numeric", length=0)
  ...
  for(i in (t+1):2) {
    ...
    P[i-1]=(P[i]-c*discount)/((1+discount)^choice[i]) # Gives P from L+1 : 1
  ...
  state[1]=int_state
  # P[i]=a - (b*choice[i])
  P[1]=(P[2]-c*discount)/((1+discount)^choice[1])
  Q[1]=(a-P[1])/b
  benefits[1]=(P[1]-c)*Q[1]
  
  for(k in 2:(t+1)) {
    state[i]=state[i-1]+choice[i] # Gives R from L+1 : 1

    benefits[i]=(P[i]-c)*choice[i] # Gives benefits from L+1 : 1
  }
...
out_1a$solution
# # Okay so of course if the time period is shorter its going to have higher benefits because you arent losing as much to discounting. Does lambda play a factor??? 
# test5=-out_1a$objective # 6288.40, L = 33
# test4=-out_1a$objective # 5997.13, L = 34
# test3=-out_1a$objective # 5199.57, L = 37
# test2=-out_1a$objective # 5718.93, L = 35
# test1=-out_1a$objective # 5453.25, L = 36 
# 
# #Po = (a*rho)/(Initial_state*discount)
